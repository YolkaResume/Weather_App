...
class WeatherAPi {
  static async getCurrentWeather(City) {
    const response = await fetch(
      `${baseUrl}${currentMode}${jsonMode}${key}${City}`
    );
  ...
  static async getDayliWeather(City, DaysCount) {
    const response = await fetch(
      `${baseUrl}${forecast}${jsonMode}${key}${City}${daysMode}${DaysCount}${forecastConditions}`
    );
    const data = await response.json();
    return data;
  }
  static async getWeaklyWeather(City) {
    const response = await fetch(
      `${baseUrl}${forecast}${jsonMode}${key}${City}${daysMode}7${forecastConditions}`
    );
    const data = await response.json();
    return data;
  }
